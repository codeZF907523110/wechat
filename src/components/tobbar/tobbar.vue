<template>
  <div class='tobbar'>
      ...
      <div class='saveusername' :class='{changegreen:username!=""}' v-if='isshowbtn==true' @click='saveusername'>
          保存
      </div>
  </div>
</template>
...
export default {
    ...
    methods:{
        saveusername(){
            this.$axios.changeusername('/changeusername',{username:this.username}).then((res)=>{
                if(res==1){
                    alert('更改成功')
                }else{
                    alert('更改失败')
